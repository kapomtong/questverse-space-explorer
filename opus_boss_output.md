...
(function() {
  ...
  const CONFIG = {
    // Movement
    PLAYER_SPEED: 320, // px/s (based on 1280px width)
  ...
  function getRandomQuestions(count) {
    ...
    planets.forEach(planetId => {
      if (QV.QUESTIONS[planetId]) {
        QV.QUESTIONS[planetId].forEach(zone => {
          zone.forEach(q => {
            allQuestions.push(q);
          });
        });
  ...
  function updatePlayerMovement(deltaTime) {
    ...
    // Apply movement
    if (vx !== 0 || vy !== 0) {
      const speed = (CONFIG.PLAYER_SPEED / window.innerWidth * 100) * deltaTime; // vw per frame
  ...
  function scheduleNextAttack() {
    ...
    const delay = CONFIG.ATTACK_INTERVAL_MIN + Math.random() * (CONFIG.ATTACK_INTERVAL_MAX - CONFIG.ATTACK_INTERVAL_MIN);
  ...
  function createFireball() {
    ...
    const attack = {
      ...
      duration: 1500, // ms warning + travel
      warned: false,
      hit: false
    };
    ...
    tile.className = 'atk-tile atk-fireball';
  ...
  function createIce() {
    ...
    // Create 3 ice drops
    for (let i = 0; i < 3; i++) {
      const x = 20 + Math.random() * 70; // random x position
      const y = -10; // start above screen
      const targetY = 85; // fall to bottom
      ...
      // Create DOM
      const tile = document.createElement('div');
      tile.className = 'atk-tile atk-ice';
      tile.style.left = x + '%';
      tile.style.top = y + '%';
  ...
  function createPortal() {
    ...
    // Create DOM
    const tile = document.createElement('div');
    tile.className = 'atk-tile atk-portal';
    tile.style.left = attack.x + '%';
    tile.style.top = attack.y + '%';
  ...
  function updateAttacks(now) {
    gameState.attacks.forEach((attack, idx) => {
      ...
      if (attack.type === 'fireball') {
        if (elapsed < 700) {
          ...
          // Travel phase
          const travelProgress = (elapsed - 700) / 800;
        ...
        if (elapsed < 800) {
          ...
          attack.drops.forEach(drop => {
            ...
            // Check collision
            if (!attack.hit && fallProgress > 0.7 && collide(drop.x, y, CONFIG.ATTACK_RADIUS, gameState.player.x, gameState.player.y, CONFIG.PLAYER_RADIUS)) {
              hitPlayer(attack);
              freezePlayer();
              attack.hit = true;
            }
        ...
        if (elapsed < 500) {
          ...
          if (!attack.hit && dist < attack.radius) {
            // Player still in portal when it triggers
            if (elapsed > 3000) {
              hitPlayer(attack);
              attack.hit = true;
            }
  ...
  function onDodge() {
    ...
    QV.saveState(QV.state);
    QV.toast(`Dodged! +${CONFIG.DODGE_XP} XP`);
    updateHUD();
  }
      ...
      hudElements.endScreen.innerHTML = `
        <div class="end-content win">
          ...
          <div class="badge-earned">
            <img src="assets/badge_${badgeId.replace('boss_', '')}.webp" alt="Badge">
            <p>Badge Earned!</p>
          </div>
